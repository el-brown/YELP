...
#4 what does this output res1[:dishes]
{ name: "Pad Thai", price: 10.25, ingredients: ["noddles", "peppers"] },
{ name: "Drunken Noodle", price: 9.25, ingredients: ["noddles", "chicken"] }
...
res_menu(res2)  #Will get you the dishes for restaurant 2
...
@all_restaurants = [res1, res2]  # Gives me restaurants in one array and makes them Hashes - READ
@all_restaurants.push(res3)  #Adds my new restaurant to the array as a Hash -CREATE

list_of_restaurants = @all_restaurants.map do |res| res[:name] end  #Returns a current list of the restaurants - READ
@all_restaurants.shift  #Shifts everything to the left and res1 falls off - DELETE
@all_restaurants[0] = "restaurant1" #Updates the name of res2 (which is now in the 0 index) to restaurant 1 -UPDATE

#BONUS Simple Array CRUD
...
#BONUS: Takes an array of restaurants and loops over them to return the res with more than 500 likes
...
# BONUS: My method to print restaurants that are open certain days
